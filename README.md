...
1. **Name the real problem.** Not the first version of the request, but the thing actually underneath it. Most requests arrive as a proposed solution ("build us a dashboard," "make an app for this"). I go looking for the workflow and the failure mode behind that ask before I agree the ask is right.
...
4. **Validate with real operators.** Not a demo audience, but the person who will actually use this thing under real conditions, with real stakes if it's wrong.
...
- **`frameworks/`**: the reasoning tools I actually use before I build anything: [Foundry](frameworks/foundry.md), the pre-build workshop I run every idea through, and [How I Build](frameworks/how-i-build.md), the shorter set of operating principles behind all of it.
